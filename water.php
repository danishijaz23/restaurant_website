...
<body>
  <?php
  if (isset($_POST['water']) && !is_array($_POST['water'])) {
    $waterBottles = isset($_POST['water']) ? $_POST['water'] : '<em>empty</em>';
  } else {
    $waterBottles = isset($_POST['water']) ? implode(', ', $_POST['water']) : '<em>empty</em>';
  }

  $waterType = isset($_POST['type'][0]) ? $_POST['type'][0] : '<em>empty</em>';
  $size = isset($_POST['size']) ? $_POST['size'] : '<em>empty</em>';
  $bottles = isset($_POST['water']) ? $_POST['water'] : '';

  if (is_array($bottles)) {
    $numberOfBottles = count($bottles);
  } else {
    $numberOfBottles = isset($_POST['water']) ? intval($_POST['water']) : 0;
  }
  ?>
  ...
  <?php if (!isset($_POST['type']) && !isset($_POST['size']) && !isset($_POST['bottles'])) { ?>
    <em>Sorry, we did not receive your information. <a href="water_form.html">Try again.</a></em>
  <?php } else { ?>
    <ul>
      <li><strong>Water Type:</strong> <?php print $waterType; ?></li>
      <li><strong>Size:</strong> <?php print $size; ?></li>
      <li><strong>Number of Water Bottles:</strong> <?php print $numberOfBottles; ?></li>
    </ul>
