...
<body>
  ...
  if (isset($_POST['flavor']) && !is_array($_POST['flavor'])) {
    $flavor = isset($_POST['flavor']) ? $_POST['flavor'] : '<em>empty</em>';
  } else {
    $flavor = isset($_POST['flavor']) ? implode(', ', $_POST['flavor']) : '<em>empty</em>';
  }
  ?>
  ...
  <h2>Your French Toast</h2>

  <?php if (!isset($_POST['flavor']) && !isset($_POST['french_toasts'])) { ?>
    <em>Sorry, we did not receive your information. <a href="french_toast_form.html">Try again.</a></em>
  <?php } else { ?>
    <ul>
      <li><strong>Falvor:</strong> <?php print $flavor; ?></li>
      <li><strong>Number of french toasts:</strong> <?php print isset($_POST['french_toasts']) && $_POST['french_toasts'] ? $_POST['french_toasts'] : '<em>empty</em>'; ?></li>
    </ul>
