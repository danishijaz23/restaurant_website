...
<body>
  <?php
  if (isset($_POST['dish']) && !is_array($_POST['dish'])) {
    $dish = isset($_POST['dish']) ? $_POST['dish'] : '<em>empty</em>';
  } else {
    $dish = isset($_POST['dish']) ? implode(', ', $_POST['dish']) : '<em>empty</em>';
  }
  ?>
  ...
  <h2>Your Egg Benedict</h2>

  <?php if (!isset($_POST['dish']) && !isset($_POST['eggs_benedicts'])) { ?>
    <em>Sorry, we did not receive your information. <a href="eggs_benedict_form.html">Try again.</a></em>
  <?php } else { ?>
    <ul>
      <li><strong>Dish:</strong> <?php print $dish; ?></li>
      <li><strong>Number of egg benedicts:</strong> <?php print isset($_POST['eggs_benedicts']) && $_POST['eggs_benedicts'] ? $_POST['eggs_benedicts'] : '<em>empty</em>'; ?></li>
    </ul>
